...
- Health dots: Green `#22C55E`, Amber `#F59E0B`, Red `#EF4444`, Gray `#D4D4D4`
- Behavior: Moving `#3B82F6`, Resting `#A78BFA`, Eating `#F59E0B`
...
### Shadows
Cards only: `0 1px 3px rgba(0,0,0,0.04)`
...
- Left: Polar bear logo (static SVG with subtle breathing animation — translateY, 4s cycle) + "Polar" in Inter 600 18px
...
- Alert state on enclosures: faint red tint `rgba(239,68,68,0.03)`, red dashed border, warning triangle SVG badge
- Animal dots: 12px Marker components, health-status colored
  - Hover (300ms delay with cancellation): scale to 16px, tooltip with "Name . Status"
...
- Sections top to bottom:
  ...
  2. **Camera feed:** Rounded container, static frame image refreshed on polling interval, red dot "Camera Feed" pill, bounding box overlays, current state pills (Moving/Resting/Eating). Click to open lightbox modal. No-data placeholder if unavailable.
